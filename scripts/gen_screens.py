...
scripts/og-fonts/ (кладёт scripts/fetch_fonts.py --og-only).
  ...
  python scripts/gen_screens.py --data data/russia.json --outdir public
"""
...
def render_narrow(data: dict, out: Path, *, title: str, eyebrow: str,
    ...
    s.label((pad, 31), "Плавание", size=10)
    ...
    y += 8
    h1 = s.sans(28, 600)
    for line in s.wrap(title.upper(), h1, right - pad, 2):
...
def render_wide(data: dict, out: Path, *, title: str, eyebrow: str,
    ...
    s.label((pad, 35), "Плавание", size=10)
...
def main() -> int:
    ...
    p.add_argument("--data", default=str(ROOT / "data" / "russia.json"))
    p.add_argument("--outdir", default=str(ROOT / "public"), type=Path)
    p.add_argument("--today", default="2026-01-01")
    args = p.parse_args()
    data = json.loads(Path(args.data).read_text(encoding="utf-8"))
    names = render(data, args.outdir, title="Рекорды России по плаванию",
