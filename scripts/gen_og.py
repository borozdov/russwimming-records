...
# Только для иконок с широкими/округлыми литерами («Ю» и т.п. — см. junior):
# при одинаковой фактической толщине штриха такие литеры визуально читаются
# легче блочных («Р»), потому что ink/bbox ниже — открытые дуги дают больше
# площади без краски при том же контуре. SemiBold этого не компенсирует, а
# Bold — да. Здесь не используется («Р» в компенсации не нуждается), константа
# держится для параллельности со scripts/gen_og.py в junior.
...
def main() -> int:
    ...
    p.add_argument("--domain", default="russwimming-records.borozdov.ru")
