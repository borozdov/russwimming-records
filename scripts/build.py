...
QR_TARGET = "https://russwimming-records.borozdov.ru/"
...
# Загрузчик стартует после load: tag.js с вебвизором (~107K gzip) — самый тяжёлый
# ресурс страницы, и из <head> он конкурировал со шрифтами и CSS за первый рендер.
# Собственных вызовов ym() до init в коде нет, очередь не теряется.
METRIKA = """
(function () {
  function start() {
...
# Титановый квадрат rx 16% + обсидиановая литера — константа бренда вне зеркала.
# Базовая линия задана числом: y = 50 + capHeight * font-size / 2 = 50 + 0.7275*62/2.
# dominant-baseline='central' здесь нельзя — он считается от ascender/descender, а
# SVG-фавикон веб-шрифты не грузит: у фолбэков (Helvetica 0.770/0.230, Helvetica Neue
# 0.975/0.217, Arial 0.905/0.212) литера уезжала до 4.6% высоты, в разные стороны.
...
def write_index(data: dict, out: Path, fetched_human: str) -> None:
    ...
    out.write_text(render_page(
        ...
        og_title=SITE_TITLE,
        description=SITE_TAGLINE,
        h1="Рекорды России по плаванию",
        eyebrow="Официальное зеркало · Всероссийская федерация плавания",
        intro=intro,
        current="",
        fetched_human=fetched_human,
        with_faq=True,
        table_h2="Таблица рекордов России по плаванию",
        caption="Действующие рекорды России по плаванию по категориям",
    ), encoding="utf-8")
...
def write_category_pages(data: dict, fetched_human: str) -> list[str]:
    ...
    for cat in data["categories"]:
        ...
        desc = (
            f"{cat['title']}: {len(recs)} действующих рекордов России по плаванию. "
            f"Обновлено {fetched_human}, за последний год обновлено {fresh}. "
            "Скачать: JSON, CSV, XLSX."
        )
        intro = (
            f"Действующие рекорды России в категории «{html.escape(cat['title'].lower())}». "
        ...
        page = render_page(
            ...
            categories=[cat],
            canonical=canonical,
            title=f"{cat['title']} — рекорды России по плаванию",
            og_title=f"{cat['title']} — рекорды России",
            description=desc,
            h1=cat["title"],
            eyebrow="Рекорды России по плаванию",
            intro=intro,
            current=cat["id"],
            fetched_human=fetched_human,
            with_faq=False,
            table_h2=f"Таблица рекордов: {cat['title']}",
            caption=f"Действующие рекорды России: {cat['title']}",
        )
