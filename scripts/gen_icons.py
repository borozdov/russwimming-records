...
def draw_letter(img: Image.Image, letter: str, color, scale: float, bold: bool = False) -> None:
    ...
    литерам с овалами, у которых есть свес за пределы капители). bold=True —
    компенсация оптического веса для широких/округлых литер вроде «Ю» в
    junior (см. gen_og.SANS_BOLD_CANDIDATES); здесь не используется, но
    сигнатура держится параллельно ради общего кода со scripts/ в junior.
    """
    w, h = img.size
...
def render(outdir: Path, letter: str, bold: bool = False) -> list[str]:
    ...
    («Ю» в junior); здесь не используется («Р» в компенсации не нуждается),
    параметр держится для общего кода. См. render() в junior для контекста.
    """
...
def main() -> int:
    ...
    p.add_argument("--letter", default="Р", help="литера (одна буква)")
